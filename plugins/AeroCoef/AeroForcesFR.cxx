...
void AeroForcesFR::setup()
{
  ...
  m_frData = fr->getData();
  //Vatsalya: OLD block kept because the FR update var can be null during DataProcessing setup for skin-friction output.
  // m_updateVarSet = m_frData->getUpdateVar().d_castTo<EulerVarSet>();
  //Vatsalya: Prefer an explicit DataProcessing*.Data.updateVar, then fall back to the FR method update var for old cases.
  if (getMethodData().getUpdateVarSetStr() != "Null") {
    m_updateVarSet = getMethodData().getUpdateVarSet().d_castTo<EulerVarSet>();
  }
  if (m_updateVarSet.isNull()) {
    m_updateVarSet = m_frData->getUpdateVar().d_castTo<EulerVarSet>();
  }
