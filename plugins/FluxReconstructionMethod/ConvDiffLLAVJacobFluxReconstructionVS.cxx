...
#include "FluxReconstructionMethod/ConvDiffLLAVJacobFluxReconstructionVS.hh"
...
ConvDiffLLAVJacobFluxReconstructionVS::ConvDiffLLAVJacobFluxReconstructionVS(const std::string& name) :
  ...
  m_trueFreezeEpsilon(false)
  {
    ...
    m_relaxUp = 0.0;
    setParameter( "TemporalRelaxUp", &m_relaxUp);

    m_relaxDown = 0.0;
    setParameter( "TemporalRelaxDown", &m_relaxDown);
    ...
    m_monitoredExpression = "";
    setParameter( "MonitoredExpression", &m_monitoredExpression);
    ...
    m_minEpsilon = 0.0;
    setParameter( "MinEpsilon", &m_minEpsilon);

    m_trueFreezeEpsilon = false;
    setParameter( "TrueFreezeEpsilon", &m_trueFreezeEpsilon);
  }

//////////////////////////////////////////////////////////////////////////////

void ConvDiffLLAVJacobFluxReconstructionVS::configure ( Config::ConfigArgs& args )
{
...
void ConvDiffLLAVJacobFluxReconstructionVS::defineConfigOptions(Config::OptionList& options)
{
  ...
  options.addConfigOption< CFreal,Config::DynamicOption<> >("TemporalRelaxUp","Temporal relaxation for epsilon increase (0=no smoothing, 1=frozen). Overrides DampingCoeff when non-zero.");

  options.addConfigOption< CFreal,Config::DynamicOption<> >("TemporalRelaxDown","Temporal relaxation for epsilon decrease (0=no smoothing, 1=frozen). Overrides DampingCoeff when non-zero.");
  ...
  options.addConfigOption< std::string >("MonitoredExpression","String name for monitored physical variable (pressure, temperature, density, velocity, soundspeed). Overrides MonitoredPhysVar when set.");
  ...
  options.addConfigOption< CFreal,Config::DynamicOption<> >("MinEpsilon","Minimum epsilon as fraction of epsilon0 (0=sensor only, 0.1=at least 10% everywhere). Prevents zero LLAV when sensor cannot detect oscillations yet.");

  options.addConfigOption< bool,Config::DynamicOption<> >("TrueFreezeEpsilon","True freeze: completely skip epsilon recomputation and reuse stored per-cell values. Set via .inter after epsilon is established.");
}

//////////////////////////////////////////////////////////////////////////////

std::vector< Common::SafePtr< BaseDataSocketSource > >
  ConvDiffLLAVJacobFluxReconstructionVS::providesSockets()
{
...
ConvDiffLLAVJacobFluxReconstructionVS::needsSockets()
{
...
void ConvDiffLLAVJacobFluxReconstructionVS::execute()
{
  CFAUTOTRACE;
  
  CFLog(VERBOSE, "ConvDiffLLAVJacobFluxReconstructionVS::execute()\n");
...
//       {
      if (m_trueFreezeEpsilon)
      {
        // True freeze: skip epsilon recomputation entirely.
        // Re-accumulate m_nodeEpsilons from stored m_cellEpsilons.
        const CFuint cellID = m_cell->getID();
        const CFreal frozenEps = m_cellEpsilons[cellID];
        m_cellEpsRelaxDeriv[cellID] = 1.0;  // no relaxation derivative when frozen
        for (CFuint iNode = 0; iNode < m_nbrCornerNodes; ++iNode)
        {
          const CFuint nodeID = (*(m_cellNodes[0]))[iNode]->getLocalID();
          m_nodeEpsilons[nodeID] += frozenEps;
          m_totalEps += frozenEps;
...
void ConvDiffLLAVJacobFluxReconstructionVS::setFaceDataForGradients(CFuint faceID)
{   
...
void ConvDiffLLAVJacobFluxReconstructionVS::setFaceData(CFuint faceID)
{   
  // get the face flux point normals
  DataHandle< CFreal > flxPntNormals = socket_flxPntNormals.getDataHandle();
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeFlxPntStatesAndGrads()
{
  // loop over flx pnts to extrapolate the states to the flux points
  for (CFuint iFlxPnt = 0; iFlxPnt < m_nbrFaceFlxPnts; ++iFlxPnt)
  {     
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeInterfaceFlxCorrection()
{
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeWaveSpeedUpdates(vector< CFreal >& waveSpeedUpd)
{
...
void ConvDiffLLAVJacobFluxReconstructionVS::initJacobianComputation()
{
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeCellFluxJacobianNum(const CFreal resFactor)
{
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeRiemannFluxJacobianNum(const CFreal resFactor)
{
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeFluxToGradJacobianNum(const CFreal resFactor)
{
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeRiemannFluxToGradJacobianNum(const CFreal resFactor)
{  
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeGradToStateJacobianAna()
{
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeGradVarsToStateJacobianNum()
{
  ...
  for (m_pertVar = 0; m_pertVar < m_nbrEqs; ++m_pertVar)
  {
    ...
    for (CFuint iEq = 0; iEq < m_nbrEqs; ++iEq)
    {
      if (m_gradVarsToStateJacobian[0][0][m_pertVar][iEq] != 0.0)
      {
        m_varToGradVarDep[m_pertVar].push_back(iEq);
      }
    }
    
    // Keep conservative full coupling if sparsity detection returns empty.
    if (m_varToGradVarDep[m_pertVar].empty())
    {
      m_varToGradVarDep[m_pertVar].reserve(m_nbrEqs);
      for (CFuint iEq = 0; iEq < m_nbrEqs; ++iEq)
      {
        m_varToGradVarDep[m_pertVar].push_back(iEq);
      }
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeLLAVCellFluxJacobianAna(const CFreal resFactor)
{
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeLLAVRiemannFluxJacobianAna(const CFreal resFactor)
{
  ...
  { 
    ...
    for (CFuint iFlx = 0; iFlx < m_nbrFaceFlxPnts; ++iFlx)
    {
      ...
      // Transform flux point states to conservative for LLAV damping
      m_tempSolVarState = *m_updateToSolutionVecTrans->transform(m_cellStatesFlxPnt[LEFT][iFlx]);
      m_tempSolVarState2 = *m_updateToSolutionVecTrans->transform(m_cellStatesFlxPnt[RIGHT][iFlx]);

      for (CFuint iVar = 0; iVar < m_nbrEqs; ++iVar)
      {
        *(m_avgGradAV[iVar]) = (*(m_cellGradFlxPntAV[LEFT][iFlx][iVar]) + *(m_cellGradFlxPntAV[RIGHT][iFlx][iVar]))/2.0;

        // compute damping term for LLAV using CONSERVATIVE variables
      ...
      for (m_pertSol = 0; m_pertSol < m_nbrSolPnts; ++m_pertSol)
      {
        ...
        //Vatsalya: compute dCons/dState Jacobian at perturbation point via in-place FD; transforms LLAV AV gradient Jacobian from diagonal (valid for Cons) to correct form for Rhoivt
        // Compute dCons/dState at flux point for LLAV damping Jacobian (in-place FD)
        {
          State* statePtr = m_cellStatesFlxPnt[m_pertSide][iFlx];
          ...
          for (CFuint j = 0; j < m_nbrEqs; ++j)
          {
            ...
            (*statePtr)[j] = origVal; // restore
          }
        }

        for (m_pertVar = 0; m_pertVar < m_nbrEqs; ++m_pertVar)
        {
          // Full dCons/dState transformation for LLAV damping Jacobian
          for (CFuint kEq = 0; kEq < m_nbrEqs; ++kEq)
          {
            m_llavRiemannFluxJacobian[m_pertSide][m_pertSol][m_pertVar][iFlx][kEq] -= l_eps_dampF_side * m_avConsToStateJac(kEq, m_pertVar);
          }
        }
      }
    }
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeBothJacobsDiffFaceTerm()
{
  ...
  for (m_pertSide = 0; m_pertSide < 2; ++m_pertSide)
  {
    ...
    if (!m_cellFlags[m_cells[m_pertSide]->getID()]) 
    {
      ...
      for (m_pertSol = 0; m_pertSol < m_nbrSolPnts; ++m_pertSol)
      {
        ...
        // Compute dCons/dState Jacobian at this perturbation point via in-place FD.
        // Transforms LLAV AV gradient Jacobian from diagonal (valid for UpdateVar=Cons)
        // to correct form for arbitrary UpdateVar (e.g. Rhoivt for CNEQ).
        {
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeOneJacobDiffFaceTerm(const CFuint side)
{
  // get residual factor
  const CFreal resFactor = getMethodData().getResFactor();
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeUnpertCellDiffResiduals(const CFuint side)
{ 
  // get datahandle
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeGradientFaceCorrections()
{ 
  // get the face flux point normals
  DataHandle< CFreal > flxPntNormals = socket_flxPntNormals.getDataHandle();
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeGradients()
{
  // get the sol pnt normals
  DataHandle< CFreal > solPntNormals = socket_solPntNormals.getDataHandle();
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeProjStates(std::vector< RealVector >& projStates)
{
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeProjStates(std::vector< RealVector >& projStates, const CFuint side)
{
  cf_assert(m_nbrSolPnts == projStates.size());
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeEpsilon()
{
  computeEpsilon0();
  ...
  // Enforce minimum epsilon floor (fraction of eps0)
  if (m_minEpsilon > 0.0)
  {
    m_epsilon = std::max(m_epsilon, m_minEpsilon * m_epsilon0);
  }
...
CFreal ConvDiffLLAVJacobFluxReconstructionVS::computePeclet()
{
  return m_peclet;
}
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeSmoothness()
{ 
  CFreal sNum = 0.0;
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeSmoothness(const CFuint side)
{ 
  CFreal sNum = 0.0;
...
void ConvDiffLLAVJacobFluxReconstructionVS::storeEpsilon()
{
  const CFuint cellID = m_cell->getID();
  const CFreal oldEps = m_cellEpsilons[cellID];

  CFreal relaxDeriv = 1.0;
  CFreal newEps = m_epsilon;

  if (!m_useMax)
  {
    if (m_relaxUp > 0.0 || m_relaxDown > 0.0)
    {
      // Asymmetric relaxation: fast up, slow down (Fix 15B)
      const CFreal beta = (m_epsilon > oldEps) ? m_relaxUp : m_relaxDown;
      newEps = (1.0 - beta) * m_epsilon + beta * oldEps;
      relaxDeriv = 1.0 - beta;
    }
    else
    {
      // Original symmetric DampingCoeff (backward compatible)
      newEps = (1.0 - m_dampingCoeff) * oldEps + m_dampingCoeff * m_epsilon;
      relaxDeriv = m_dampingCoeff;
    }
  }

  m_cellEpsRelaxDeriv[cellID] = relaxDeriv;
  ...
  for (CFuint iNode = 0; iNode < m_nbrCornerNodes; ++iNode)
  {
    ...
    if (!m_useMax)
    {
      ...
      m_cellEpsilons[cellID] = newEps;
      m_totalEps += newEps;
    }
    else
    {
      const CFreal maxEps = max(m_epsilon, m_cellEpsilons[cellID]);
      m_nodeEpsilons[nodeID] += maxEps;
      m_cellEpsilons[cellID] = maxEps;
      m_totalEps += maxEps;
    }
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeFlux(const RealVector& values, const std::vector< RealVector* >& gradients, const RealVector& normal, const CFreal& radius, RealVector& flux)
{
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeEpsilon0()
{ 
...
void ConvDiffLLAVJacobFluxReconstructionVS::computeEpsilon0(const CFuint side)
{ 
  const CFreal wavespeed = 1;
...
void ConvDiffLLAVJacobFluxReconstructionVS::setup()
{
  CFAUTOTRACE;
  ...
  m_avConsToStateJac.resize(m_nbrEqs, m_nbrEqs);
  m_origConsState.resize(m_nbrEqs);
  m_nodeEpsilons.resize(nbrNodes);
  m_nbNodeNeighbors.resize(nbrNodes);
  m_cellEpsilons.resize(nbrCells);
  m_cellEpsRelaxDeriv.resize(nbrCells);
  ...
  // Resolve MonitoredExpression string to MonitoredPhysVar index
  if (m_monitoredExpression != "")
  {
    // EulerTerm physical data indices (NavierStokes/EulerTerm.hh)
    if      (m_monitoredExpression == "pressure"   || m_monitoredExpression == "p") m_monitoredPhysVar = 1;
    else if (m_monitoredExpression == "temperature" || m_monitoredExpression == "T") m_monitoredPhysVar = 5;
    else if (m_monitoredExpression == "density"     || m_monitoredExpression == "rho") m_monitoredPhysVar = 0;
    else if (m_monitoredExpression == "velocity"    || m_monitoredExpression == "V") m_monitoredPhysVar = 6;
    else if (m_monitoredExpression == "soundspeed"  || m_monitoredExpression == "a") m_monitoredPhysVar = 4;
    else if (m_monitoredExpression == "enthalpy"    || m_monitoredExpression == "H") m_monitoredPhysVar = 2;
    else if (m_monitoredExpression == "energy"      || m_monitoredExpression == "E") m_monitoredPhysVar = 3;
    else
    {
      CFLog(WARN, "ConvDiffLLAVJacobFluxReconstructionVS::setup() - Unknown MonitoredExpression: "
            << m_monitoredExpression << ". Falling back to MonitoredVar = " << m_monitoredVar << "\n");
    }

    if (m_monitoredPhysVar != MathTools::MathConsts::CFuintMax())
    {
      CFLog(INFO, "ConvDiffLLAVJacobFluxReconstructionVS::setup() - MonitoredExpression: "
            << m_monitoredExpression << " -> MonitoredPhysVar = " << m_monitoredPhysVar << "\n");
    }
  }

  // Only assert state-index bound when NOT using physical-data monitoring
  if (m_monitoredPhysVar == MathTools::MathConsts::CFuintMax())
  {
...
void ConvDiffLLAVJacobFluxReconstructionVS::unsetup()
{
  CFAUTOTRACE;
