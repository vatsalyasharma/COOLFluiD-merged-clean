...
#include "FluxReconstructionMethod/ConvDiffLLAVFluxReconstructionVS.hh"
...
MethodCommandProvider< ConvDiffLLAVFluxReconstructionVS,
		       FluxReconstructionSolverData,
		       FluxReconstructionModule >
convDiffLLAVRHSVSFluxReconstructionProvider("ConvDiffLLAVRHSVS");

//////////////////////////////////////////////////////////////////////////////

ConvDiffLLAVFluxReconstructionVS::ConvDiffLLAVFluxReconstructionVS(const std::string& name) :
...
void ConvDiffLLAVFluxReconstructionVS::configure ( Config::ConfigArgs& args )
{
...
void ConvDiffLLAVFluxReconstructionVS::defineConfigOptions(Config::OptionList& options)
{
...
std::vector< Common::SafePtr< BaseDataSocketSource > >
  ConvDiffLLAVFluxReconstructionVS::providesSockets()
{
...
ConvDiffLLAVFluxReconstructionVS::needsSockets()
{
...
void ConvDiffLLAVFluxReconstructionVS::execute()
{
  CFAUTOTRACE;

  CFLog(VERBOSE, "ConvDiffLLAVFluxReconstructionVS::execute()\n");
  ...
  for (m_iElemType = 0; m_iElemType < nbrElemTypes; ++m_iElemType)
  {
    ...
    for (CFuint elemIdx = startIdx; elemIdx < endIdx; ++elemIdx)
    {
      ...
      {
        // True freeze (Fix 22 port): skip epsilon recomputation entirely.
...
void ConvDiffLLAVFluxReconstructionVS::setFaceDataForGradients(CFuint faceID)
{
...
void ConvDiffLLAVFluxReconstructionVS::setFaceData(CFuint faceID)
{
  // get the face flux point normals
  DataHandle< CFreal > flxPntNormals = socket_flxPntNormals.getDataHandle();
...
void ConvDiffLLAVFluxReconstructionVS::computeFlxPntStatesAndGrads()
{
  // loop over flx pnts to extrapolate the states to the flux points
  for (CFuint iFlxPnt = 0; iFlxPnt < m_nbrFaceFlxPnts; ++iFlxPnt)
  {
...
void ConvDiffLLAVFluxReconstructionVS::computeInterfaceFlxCorrection()
{
...
void ConvDiffLLAVFluxReconstructionVS::computeWaveSpeedUpdates(vector< CFreal >& waveSpeedUpd)
{
...
void ConvDiffLLAVFluxReconstructionVS::computeUnpertCellDiffResiduals(const CFuint side)
{
...
void ConvDiffLLAVFluxReconstructionVS::computeGradientFaceCorrections()
{
  // get the face flux point normals
  DataHandle< CFreal > flxPntNormals = socket_flxPntNormals.getDataHandle();
...
void ConvDiffLLAVFluxReconstructionVS::computeGradients()
{
  // get the sol pnt normals
  DataHandle< CFreal > solPntNormals = socket_solPntNormals.getDataHandle();
...
void ConvDiffLLAVFluxReconstructionVS::computeProjStates(std::vector< RealVector >& projStates)
{
...
void ConvDiffLLAVFluxReconstructionVS::computeProjStates(std::vector< RealVector >& projStates, const CFuint side)
{
  cf_assert(m_nbrSolPnts == projStates.size());
...
void ConvDiffLLAVFluxReconstructionVS::computeEpsilon()
{
  computeEpsilon0();
...
CFreal ConvDiffLLAVFluxReconstructionVS::computePeclet()
{
  return m_peclet;
}
...
void ConvDiffLLAVFluxReconstructionVS::computeSmoothness()
{
  CFreal sNum = 0.0;
...
void ConvDiffLLAVFluxReconstructionVS::computeSmoothness(const CFuint side)
{
  CFreal sNum = 0.0;
...
void ConvDiffLLAVFluxReconstructionVS::storeEpsilon()
{
  const CFuint cellID = m_cell->getID();
  ...
  if (!m_useMax)
  {
    ...
    {
      // Asymmetric relaxation: fast up, slow down (Fix 15B port)
...
void ConvDiffLLAVFluxReconstructionVS::computeFlux(const RealVector& values, const std::vector< RealVector* >& gradients, const RealVector& normal, const CFreal& radius, RealVector& flux)
{
...
void ConvDiffLLAVFluxReconstructionVS::computeEpsilon0()
{
...
void ConvDiffLLAVFluxReconstructionVS::computeEpsilon0(const CFuint side)
{
  const CFreal wavespeed = 1;
...
void ConvDiffLLAVFluxReconstructionVS::setup()
{
  CFAUTOTRACE;
  ...
  {
    // EulerTerm physical data indices (Framework/EulerTerm.hh)
    ...
    {
      CFLog(WARN, "ConvDiffLLAVFluxReconstructionVS::setup() - Unknown MonitoredExpression: "
            << m_monitoredExpression << ". Falling back to MonitoredVar = " << m_monitoredVar << "\n");
    }

    if (m_monitoredPhysVar != MathTools::MathConsts::CFuintMax())
    {
      CFLog(INFO, "ConvDiffLLAVFluxReconstructionVS::setup() - MonitoredExpression: "
...
void ConvDiffLLAVFluxReconstructionVS::unsetup()
{
  CFAUTOTRACE;
