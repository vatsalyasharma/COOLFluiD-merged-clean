...
#ifndef COOLFluiD_FluxReconstructionMethod_ConvDiffLLAVJacobFluxReconstructionVS_hh
#define COOLFluiD_FluxReconstructionMethod_ConvDiffLLAVJacobFluxReconstructionVS_hh
...
class ConvDiffLLAVJacobFluxReconstructionVS : public DiffRHSJacobFluxReconstruction {

public: // functions

  /// Constructor
  explicit ConvDiffLLAVJacobFluxReconstructionVS(const std::string& name);

  /// Destructor
  virtual ~ConvDiffLLAVJacobFluxReconstructionVS() {}
...
protected: //functions
  ...
  //Vatsalya: original used diagonal approximation m_tempFlux[m_pertVar]+=value (only correct for UpdateVar=Cons); for Rhoivt the Cons->State transformation has cross-coupling through rho*u, rho*v, rho*E; now uses full dCons/dState matrix via FD
  /**
   * Add LLAV Jacobian contribution with dCons/dState transformation.
   * Replaces the diagonal approximation: m_tempFlux[m_pertVar] += value
   * with the full transformation: m_tempFlux[k] += value * dCons(k,m_pertVar)
   * for all equations k. This is needed when AV gradients use conservative
   * variables but UpdateVar != Cons (e.g. Rhoivt for CNEQ).
   */
  void addLLAVJacobToFlux(const CFreal value)
  {
    for (CFuint kEq = 0; kEq < m_nbrEqs; ++kEq)
    {
      m_tempFlux[kEq] += value * m_avConsToStateJac(kEq, m_pertVar);
    }
  }
...
protected: //data
  ...
  /// temporal relaxation for epsilon increase (0=no smoothing, 1=frozen)
  CFreal m_relaxUp;

  /// temporal relaxation for epsilon decrease (0=no smoothing, 1=frozen)
  CFreal m_relaxDown;
  ...
  /// d(epsilon_relaxed)/d(epsilon_raw) used for Jacobian consistency
  RealVector m_cellEpsRelaxDeriv; //Vatsalya: original Jacobian used derivatives of epsilon_raw while residual used epsilon_relaxed; stored d(eps_relaxed)/d(eps_raw) per cell to scale Jacobian consistently
  ...
  /// string name for monitored physical variable (overrides MonitoredPhysVar when set)
  std::string m_monitoredExpression;
  ...
  /// Minimum epsilon as fraction of epsilon0 (0 = no minimum, 1 = always full LLAV)
  CFreal m_minEpsilon;
  ...
  /// Jacobian matrix dCons/dState for LLAV AV gradient transformation
  /// Computed numerically via FD perturbation of m_updateToSolutionVecTrans
  RealMatrix m_avConsToStateJac;

  /// Buffer for unperturbed conservative state (FD computation)
  RealVector m_origConsState;

  /// True freeze: completely skip epsilon recomputation and reuse stored per-cell values
  bool m_trueFreezeEpsilon; //Vatsalya: existing FreezeLimiterIter only applied ratchet (max of old,new); epsilon-solution feedback still created ~2000-iter limit cycle; TrueFreezeEpsilon skips epsilon recomputation entirely, making LLAV a fixed linear operator
...
}; // class ConvDiffLLAVJacobFluxReconstructionVS
...
#endif // COOLFluiD_FluxReconstructionMethod_ConvDiffLLAVJacobFluxReconstructionVS_hh
