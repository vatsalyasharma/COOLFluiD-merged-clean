...
void FluxReconstructionSolverData::defineConfigOptions(Config::OptionList& options)
{
  ...
  options.addConfigOption< bool >("FreezeDiffCoeff","Freeze diffusive coefficients (mu, lambda) during Jacobian perturbation to remove noisy dmu/dT contributions.");
  options.addConfigOption< std::string >("DiffFluxFormulation","Diffusive flux formulation: BR2 (default) or BR2VS (average-of-fluxes).");
  options.addConfigOption< std::string >("SolToUpdateInUpdateMatTrans","Optional explicit solution-to-update matrix transformer provider in update variables.");
}
...
FluxReconstructionSolverData::FluxReconstructionSolverData(Common::SafePtr<Framework::Method> owner) :
  ...
  m_solToUpdateInUpdateMatTransStr(),
  ...
  m_pointDiagBlocks(CFNULL)
{
  ...
  m_freezeDiffCoeff = false;
  setParameter( "FreezeDiffCoeff", &m_freezeDiffCoeff );
  ...
  m_diffFluxFormulation = "BR2";
  setParameter( "DiffFluxFormulation", &m_diffFluxFormulation );

  m_solToUpdateInUpdateMatTransStr = "";
  setParameter( "SolToUpdateInUpdateMatTrans", &m_solToUpdateInUpdateMatTransStr );
}
...
void FluxReconstructionSolverData::setup()
{
  ...
  const bool explicitSolToUpdateMatTrans = !m_solToUpdateInUpdateMatTransStr.empty();
  std::string solToUpdateInUpdateMatTransStr = explicitSolToUpdateMatTrans ?
    m_solToUpdateInUpdateMatTransStr :
  ...
  CFLog(VERBOSE, "Configuring VarSet Transformer: " <<
	...
	(explicitSolToUpdateMatTrans ? " (explicit)" : "") << "\n");
  ...
  try {
    ...
    if (explicitSolToUpdateMatTrans)
    {
      throw;
    }
