...
void LLAVFluxReconstruction::defineConfigOptions(Config::OptionList& options)
{
  ...
  options.addConfigOption< CFreal,Config::DynamicOption<> >("TemporalRelaxUp","Temporal relaxation for epsilon increase (0=no smoothing, 1=frozen).");

  options.addConfigOption< CFreal,Config::DynamicOption<> >("TemporalRelaxDown","Temporal relaxation for epsilon decrease (0=no smoothing, 1=frozen).");
...
//       {
      ...
      {
        // True freeze: skip epsilon recomputation entirely and re-accumulate stored cell epsilons.
        const CFreal frozenEps = m_cellEpsilons[m_cell->getID()];
  ...
  for (CFuint iFace = 0; iFace < nbrFaces; ++iFace)
  {
	...
	// set unit normal vector
	  ...
	  *(m_cellStatesFlxPnt[0][iFlxPnt]) += coeff*(*((*(m_cellStates))[solIdx]));
      ...
      if ((getMethodData().getUpdateVarStr() == "Cons" || getMethodData().getUpdateVarStr() == "RhoivtTv" || getMethodData().getUpdateVarStr() == "Rhoivt") && getMethodData().hasDiffTerm())
      {
...
{ 
  for (CFuint iNode = 0; iNode < m_nbrCornerNodes; ++iNode)
  {
    ...
    if (!m_useMax) 
    {
      ...
      {
        // Asymmetric relaxation: fast up, slow down.
        const CFreal oldEps = m_cellEpsilons[m_cell->getID()];
...
void LLAVFluxReconstruction::setup()
{
  ...
  if (m_monitoredExpression != "")
  {
    ...
    {
      CFLog(WARN, "LLAVFluxReconstruction::setup() - Unknown MonitoredExpression: "
            << m_monitoredExpression << ". Falling back to MonitoredVar = " << m_monitoredVar << "\n");
    }

    if (m_monitoredPhysVar != MathTools::MathConsts::CFuintMax())
    {
      CFLog(INFO, "LLAVFluxReconstruction::setup() - MonitoredExpression: "
