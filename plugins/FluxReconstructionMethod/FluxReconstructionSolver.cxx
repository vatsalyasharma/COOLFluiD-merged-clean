...
void FluxReconstructionSolver::defineConfigOptions(Config::OptionList& options)
{
  ...
  options.addConfigOption< std::string >("ConvBndCorrectionCom","Optional explicit convective boundary correction provider.");
  options.addConfigOption< std::string >("DiffBndCorrectionCom","Optional explicit diffusive boundary correction provider.");
  ...
  options.addConfigOption< std::string >("BlendingType","Blending type when UseBlending=true (OrderBlending or SubcellFVVS).");
  ...
  m_bcsDiffComs()
{
  ...
  m_convBndCorrectionStr = "";
  setParameter("ConvBndCorrectionCom", &m_convBndCorrectionStr);

  m_diffBndCorrectionStr = "";
  setParameter("DiffBndCorrectionCom", &m_diffBndCorrectionStr);
  
  m_useBlending = false;
  setParameter("UseBlending", &m_useBlending);

  m_blendingTypeStr = "OrderBlending";
  setParameter("BlendingType", &m_blendingTypeStr);
...
void FluxReconstructionSolver::configure ( Config::ConfigArgs& args )
{
  ...
  bool useSubcellFV = false;
  if (m_useBlending)
  {
    if (m_blendingTypeStr == "SubcellFVVS" || m_blendingTypeStr == "SubcellFV")
    {
      useSubcellFV = true;
    }
    else if (!m_blendingTypeStr.empty() && m_blendingTypeStr != "OrderBlending" && m_blendingTypeStr != "Blending")
    {
      CFLog(WARN, "Unknown BlendingType '" << m_blendingTypeStr
            << "'. Falling back to OrderBlending.\n");
    }
  }

  const std::string convSuffix = (m_useBlending ? (useSubcellFV ? "SubcellFVVS" : "Blending") : "");
  const std::string convProvider = m_useBlending ? (m_convSolveStr + convSuffix) : m_convSolveStr;
  configureCommand< FluxReconstructionSolverData,FluxReconstructionSolverCom::PROVIDER >(
    args, m_convSolve,convProvider,m_data );
  configureCommand< FluxReconstructionSolverData,FluxReconstructionSolverCom::PROVIDER >(
...
void FluxReconstructionSolver::configureBcCommands ( Config::ConfigArgs& args )
{
  ...
  // configure commands
    ...
    const bool useSubcellFV = (m_useBlending &&
      (m_blendingTypeStr == "SubcellFVVS" || m_blendingTypeStr == "SubcellFV"));
    const bool useConvBndBlending = (m_useBlending && !useSubcellFV);
    ...
    for(CFuint iBc = 0; iBc < m_bcsComs.size(); ++iBc)
    {
      ...
      const bool explicitConvBndProvider = !m_convBndCorrectionStr.empty();
      std::string convBndCorrectionStr = explicitConvBndProvider ?
        m_convBndCorrectionStr :
        "ConvBndCorrections" +
          (useConvBndBlending ? m_spaceRHSJacobStr + "Blending" : m_spaceRHSJacobStr);
      ...
      {
        if (explicitConvBndProvider)
        {
          throw;
        }
        CFLog(INFO, e.what() << "\n");
        std::string fallbackStr = std::string("ConvBndCorrectionsRHS") + (useConvBndBlending ? "Blending" : "");
      ...
      std::string diffPhysicsSuffix = "";
      bool diffUsesVSProvider = false;
      {
        const std::string diffPrefix = "DiffRHS";
        if (m_diffSolveStr.length() > diffPrefix.length() &&
            m_diffSolveStr.substr(0, diffPrefix.length()) == diffPrefix)
        {
          diffPhysicsSuffix = m_diffSolveStr.substr(diffPrefix.length());

          const std::string blendingVS = "BlendingVS";
          const std::string blending = "Blending";
          if (diffPhysicsSuffix.length() >= blendingVS.length() &&
              diffPhysicsSuffix.substr(diffPhysicsSuffix.length() - blendingVS.length()) == blendingVS)
          {
            diffPhysicsSuffix = diffPhysicsSuffix.substr(0, diffPhysicsSuffix.length() - blendingVS.length());
            diffUsesVSProvider = true;
          }
          else if (diffPhysicsSuffix.length() >= blending.length() &&
                   diffPhysicsSuffix.substr(diffPhysicsSuffix.length() - blending.length()) == blending)
          {
            diffPhysicsSuffix = diffPhysicsSuffix.substr(0, diffPhysicsSuffix.length() - blending.length());
          }
          else if (diffPhysicsSuffix.length() >= 2 &&
                   diffPhysicsSuffix.substr(diffPhysicsSuffix.length() - 2) == "VS")
          {
            diffPhysicsSuffix = diffPhysicsSuffix.substr(0, diffPhysicsSuffix.length() - 2);
            diffUsesVSProvider = true;
          }
        }
      }

      const bool needPhysicsInject = (!diffPhysicsSuffix.empty() &&
        m_spaceRHSJacobStr.find(diffPhysicsSuffix) == std::string::npos);
      const bool isJacobMode = (m_spaceRHSJacobStr.find("Jacob") != std::string::npos);
      const std::string baseJacob = "DiffBndCorrections" + m_spaceRHSJacobStr;
      const std::string baseRHS   = "DiffBndCorrectionsRHS";
      const std::string physicsSuffix = diffPhysicsSuffix + (diffUsesVSProvider ? "VS" : "");
      const std::string physicsBlendingSuffix = diffPhysicsSuffix + "Blending" + (diffUsesVSProvider ? "VS" : "");
      const std::string genericBlendingSuffix = std::string("Blending") + (diffUsesVSProvider ? "VS" : "");

      bool diffBndConfigured = false;
      std::string diffBndSelectedProvider;
      std::vector<std::string> diffBndCandidates;

      if (!m_diffBndCorrectionStr.empty())
      {
        configureCommand<FluxReconstructionSolverCom,
          FluxReconstructionSolverData,
          FluxReconstructionSolverComProvider>
          (args, m_bcsDiffComs[iBc], m_diffBndCorrectionStr, m_bcNameDiffStr[iBc], m_data);
        diffBndConfigured = true;
        diffBndSelectedProvider = m_diffBndCorrectionStr;
      }
      else
      {
        if (m_useBlending)
        {
          if (needPhysicsInject) diffBndCandidates.push_back(baseJacob + physicsBlendingSuffix);
          diffBndCandidates.push_back(baseJacob + genericBlendingSuffix);
          if (diffUsesVSProvider) diffBndCandidates.push_back(baseJacob + "Blending");
          else diffBndCandidates.push_back(baseJacob + "BlendingVS");
        }
        if (needPhysicsInject) diffBndCandidates.push_back(baseJacob + physicsSuffix);
        diffBndCandidates.push_back(baseJacob);
        if (m_useBlending)
        {
          if (needPhysicsInject) diffBndCandidates.push_back(baseRHS + physicsBlendingSuffix);
          diffBndCandidates.push_back(baseRHS + genericBlendingSuffix);
          if (diffUsesVSProvider) diffBndCandidates.push_back(baseRHS + "Blending");
          else diffBndCandidates.push_back(baseRHS + "BlendingVS");
        }
        if (needPhysicsInject) diffBndCandidates.push_back(baseRHS + physicsSuffix);
        diffBndCandidates.push_back(baseRHS);

        for (CFuint iCand = 0; iCand < diffBndCandidates.size() && !diffBndConfigured; ++iCand)
        {
          const std::string& provName = diffBndCandidates[iCand];
          try
          {
            configureCommand<FluxReconstructionSolverCom,
              FluxReconstructionSolverData,
              FluxReconstructionSolverComProvider>
              (args, m_bcsDiffComs[iBc], provName, m_bcNameDiffStr[iBc], m_data);
            diffBndConfigured = true;
            diffBndSelectedProvider = provName;
          }
          catch (Common::NoSuchValueException& e)
          {
            CFLog(VERBOSE, "Provider '" << provName << "' not found\n");
          }
        }
      }

      if (!diffBndConfigured)
      {
        std::string triedList;
        for (CFuint iCand = 0; iCand < diffBndCandidates.size(); ++iCand)
        {
          triedList += " " + diffBndCandidates[iCand];
        }
        throw Common::NoSuchValueException(FromHere(),
          "No diffusive boundary provider found. Tried:" + triedList);
      }

      CFLog(INFO, "Diffusive boundary: using provider '" << diffBndSelectedProvider << "'\n");
      if (isJacobMode && diffBndSelectedProvider.find("Jacob") == std::string::npos)
      {
        CFLog(WARN, "Diffusive boundary using RHS-only provider '"
              << diffBndSelectedProvider
              << "'. Boundary faces contribute no diffusive Jacobian entries.\n");
      }
      if (m_useBlending && diffBndSelectedProvider.find("Blending") == std::string::npos)
      {
        CFLog(WARN, "Diffusive boundary using non-blended provider '"
              << diffBndSelectedProvider
              << "'. Boundary diffusive corrections will not be scaled by blending.\n");
      }
