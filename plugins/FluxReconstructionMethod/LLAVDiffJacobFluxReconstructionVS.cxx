...
#include "FluxReconstructionMethod/LLAVDiffJacobFluxReconstructionVS.hh"
...
MethodCommandProvider< LLAVDiffJacobFluxReconstructionVS,
		       FluxReconstructionSolverData,
		       FluxReconstructionModule >
LLAVDiffJacobFluxReconstructionVSFluxReconstructionProvider("LLAVDiffJacobVS");

//////////////////////////////////////////////////////////////////////////////

LLAVDiffJacobFluxReconstructionVS::LLAVDiffJacobFluxReconstructionVS(const std::string& name) :
...
void LLAVDiffJacobFluxReconstructionVS::defineConfigOptions(Config::OptionList& options)
{
}

//////////////////////////////////////////////////////////////////////////////

void LLAVDiffJacobFluxReconstructionVS::configure ( Config::ConfigArgs& args )
{
...
void LLAVDiffJacobFluxReconstructionVS::execute()
{
  CFAUTOTRACE;

  CFLog(VERBOSE, "LLAVDiffJacobFluxReconstructionVS::execute()\n");
...
void LLAVDiffJacobFluxReconstructionVS::computeInterfaceFlxCorrection()
{
...
void LLAVDiffJacobFluxReconstructionVS::computeWaveSpeedUpdates(vector< CFreal >& waveSpeedUpd)
{
...
void LLAVDiffJacobFluxReconstructionVS::computeDivDiscontFlx(vector< RealVector >& residuals)
{
  ...
  for (CFuint iFace = 0; iFace < nbrFaces; ++iFace)
  {
	...
	m_unitNormalFlxPnts2[iFlxPnt] = (m_mappedFaceNormalDir*faceJacobVecs[iFlxPnt]/faceJacobVecAbsSizeFlxPnts);
	  ...
	  *(m_cellStatesFlxPnt[0][iFlxPnt]) += coeff*(*((*(m_cellStates))[solIdx]));
      ...
      if ((getMethodData().getUpdateVarStr() == "Cons" || (getMethodData().getUpdateVarStr() == "RhoivtTv" || getMethodData().getUpdateVarStr() == "Rhoivt")) && getMethodData().hasDiffTerm()) //Vatsalya: original checked only "RhoivtTv" (TCNEQ 2T); CNEQ uses "Rhoivt" (1T) which bypassed all NEQ-specific gradient/composition/physicality paths
      {
	for (CFuint iFlxPnt = 0; iFlxPnt < m_nbrFaceFlxPnts; ++iFlxPnt)
        {
	  for (CFuint iVar = 0; iVar < m_nbrEqs; ++iVar)
...
void LLAVDiffJacobFluxReconstructionVS::computeFlux(const RealVector& values, const std::vector< RealVector* >& gradients, const RealVector& normal, const CFreal& radius, RealVector& flux)
{
...
void LLAVDiffJacobFluxReconstructionVS::computeUnpertCellDiffResiduals()
{
  ...
  for (CFuint iSide = 0; iSide < 2; ++iSide)
  {
    ...
    *m_cellStates = *(m_states[iSide]);
    ...
    const CFuint pertSideBackup = m_pertSide;
    m_pertSide = iSide;
    computeDivDiscontFlx(m_pertDivContFlx[0]);
    m_pertSide = pertSideBackup;
    ...
    // Vatsalya: the FR copy called computeFaceRes() for other internal faces,
    // but that helper is absent from the May Jacobian infrastructure.
    // Keep the May/base treatment here and add only the current face term.
...
void LLAVDiffJacobFluxReconstructionVS::computePertCellDiffResiduals(const CFuint side)
{
//   for (CFuint iState = 0; iState < m_nbrSolPnts; ++iState)
//   {
...
//   }
  ...
  *m_cellStates = *(m_states[side]);
  ...
  // Vatsalya: see computeUnpertCellDiffResiduals(); the undefined FR
  // computeFaceRes() hook is intentionally not called in the May merge.
  if (m_cells[side]->getID() == 1944) CFLog(VERBOSE, "pert res: " << m_pertCellDiffRes << "\n");
}

//////////////////////////////////////////////////////////////////////////////

void LLAVDiffJacobFluxReconstructionVS::setup()
{
  CFAUTOTRACE;
...
void LLAVDiffJacobFluxReconstructionVS::unsetup()
{
  CFAUTOTRACE;
