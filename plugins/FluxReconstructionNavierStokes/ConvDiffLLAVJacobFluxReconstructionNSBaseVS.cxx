...
ConvDiffLLAVJacobFluxReconstructionNSBaseVS::ConvDiffLLAVJacobFluxReconstructionNSBaseVS(const std::string& name) :
  ConvDiffLLAVJacobFluxReconstructionVS(name),
  m_tempGradTerm(),
  m_tempGradTermL(),
  m_tempGradTermR(),
...
void ConvDiffLLAVJacobFluxReconstructionNSBaseVS::configure ( Config::ConfigArgs& args )
{
  ConvDiffLLAVJacobFluxReconstructionVS::configure(args);
} 
  
//////////////////////////////////////////////////////////////////////////////

void ConvDiffLLAVJacobFluxReconstructionNSBaseVS::computeWaveSpeedUpdates(vector< CFreal >& waveSpeedUpd)
{
  ...
  ConvDiffLLAVJacobFluxReconstructionVS::computeWaveSpeedUpdates(waveSpeedUpd);
...
void ConvDiffLLAVJacobFluxReconstructionNSBaseVS::computeInterfaceFlxCorrection()
{
  ...
  { 
    ...
    // Transform flux point states to conservative for LLAV AV gradient damping
    m_tempSolVarState = *m_updateToSolutionVecTrans->transform(m_cellStatesFlxPnt[LEFT][iFlxPnt]);
    m_tempSolVarState2 = *m_updateToSolutionVecTrans->transform(m_cellStatesFlxPnt[RIGHT][iFlxPnt]);
    ...
    for (CFuint iGrad = 0; iGrad < m_nbrEqs; ++iGrad)
    {
      ...
      *m_avgGrad[iGrad] -= dampFactor*dGradVarXNormal;
      ...
      // compute damping term for LLAV using conservative (solution) variables
...
void ConvDiffLLAVJacobFluxReconstructionNSBaseVS::computeRiemannFluxJacobianNum(const CFreal resFactor)
{
...
void ConvDiffLLAVJacobFluxReconstructionNSBaseVS::computeRiemannFluxToGradJacobianNum(const CFreal resFactor)
{  
...
void ConvDiffLLAVJacobFluxReconstructionNSBaseVS::computeGradVarsToStateJacobianNum()
{
  ...
  for (m_pertVar = 0; m_pertVar < m_nbrEqs; ++m_pertVar)
  {
    ...
    for (CFuint iEq = 0; iEq < m_nbrEqs; ++iEq)
    {
      ...
      for (CFuint iSide = 0; iSide < 2 && !hasDependency; ++iSide)
      {
        for (CFuint iSol = 0; iSol < m_nbrSolPnts; ++iSol)
        {
          if (std::abs(m_gradVarsToStateJacobian[iSide][iSol][m_pertVar][iEq]) > depTol)
          {
            hasDependency = true;
            break;
          }
    ...
    // Fallback: if numerical dependency detection is too strict, keep a conservative full coupling.
    if (m_nbrVarToGradVarDep[m_pertVar] == 0)
    {
      ...
      m_nbrVarToGradVarDep[m_pertVar] = m_nbrEqs;
    }
  }
}
...
void ConvDiffLLAVJacobFluxReconstructionNSBaseVS::computeBndGradTerms(RealMatrix& gradTerm, RealMatrix& ghostGradTerm)
{ 
  for (CFuint iFlx = 0; iFlx < m_nbrFaceFlxPnts; ++iFlx)
  {
...
void ConvDiffLLAVJacobFluxReconstructionNSBaseVS::computeCellGradTerm(RealMatrix& gradTerm)
{   
  for (CFuint iSol = 0; iSol < m_nbrSolPnts; ++iSol)
  {
...
void ConvDiffLLAVJacobFluxReconstructionNSBaseVS::computeFaceGradTerms(RealMatrix& gradTermL, RealMatrix& gradTermR)
{
  for (CFuint iFlx = 0; iFlx < m_nbrFaceFlxPnts; ++iFlx)
  {
...
void ConvDiffLLAVJacobFluxReconstructionNSBaseVS::prepareFluxComputation()
{
  ...
  m_diffusiveVarSetNS->setComposition(m_avgSol, effectivePerturb, iPerturbVar);
}

//////////////////////////////////////////////////////////////////////////////

void ConvDiffLLAVJacobFluxReconstructionNSBaseVS::computeGradients()
{
  ...
  // compute the gradients for the artificial viscosity (using conservative/solution variables)
    for (CFuint iSol = 0; iSol < m_nbrSolPnts; ++iSol)
    {
      for (CFuint iEq = 0; iEq < m_nbrEqs; ++iEq)
      {
    ...
    for (CFuint iSolPnt = 0; iSolPnt < m_nbrSolPnts; ++iSolPnt)
    {
      ...
      // Transform to conservative (solution) variables for AV gradients
      const RealVector& consState = *m_updateToSolutionVecTrans->transform((*m_cellStates)[iSolPnt]);
      ...
      for (CFuint iEq = 0; iEq < m_nbrEqs; ++iEq)
      {
        ...
        for (CFuint iDir = 0; iDir < m_dim; ++iDir)
        {
          for (CFuint jDir = 0; jDir < m_dim; ++jDir)
          {
	    // project the conservative state on a normal
	    m_projectedCorrL[jDir] = consState[iEq] * solPntNormals[solID*m_dim*m_dim+iDir*m_dim+jDir];
          }
...
void ConvDiffLLAVJacobFluxReconstructionNSBaseVS::computeGradientFaceCorrections()
{
  ...
  // compute the gradients for the artificial viscosity
    ...
    // AV gradients use conservative (solution) variables
    for (CFuint iFlx = 0; iFlx < m_nbrFaceFlxPnts; ++iFlx)
    {
      const CFuint flxIdxL = (*m_faceFlxPntConnPerOrient)[m_orient][LEFT][iFlx];
      const CFuint flxIdxR = (*m_faceFlxPntConnPerOrient)[m_orient][RIGHT][iFlx];

      // Transform flux point states to conservative for AV gradient corrections
...
void ConvDiffLLAVJacobFluxReconstructionNSBaseVS::computeSmoothness()
{
  CFreal sNum = 0.0;
  ...
  const bool RhoivtTv = (getMethodData().getUpdateVarStr() == "RhoivtTv" || getMethodData().getUpdateVarStr() == "Rhoivt"); //Vatsalya: original checked only "RhoivtTv" (TCNEQ 2T); CNEQ uses "Rhoivt" (1T) which bypassed all NEQ-specific gradient/composition/physicality paths
...
void ConvDiffLLAVJacobFluxReconstructionNSBaseVS::computeSmoothness(const CFuint side)
{ 
  CFreal sNum = 0.0;
  ...
  const bool RhoivtTv = (getMethodData().getUpdateVarStr() == "RhoivtTv" || getMethodData().getUpdateVarStr() == "Rhoivt");
...
void ConvDiffLLAVJacobFluxReconstructionNSBaseVS::computeEpsilon0()
{ 
...
void ConvDiffLLAVJacobFluxReconstructionNSBaseVS::computeEpsilon0(const CFuint side)
{ 
  // compute a cell average characteristic flow speed. Note that a straight average is used, not a weighted one, maybe change this
  CFreal wavespeed = 0.0;
...
void ConvDiffLLAVJacobFluxReconstructionNSBaseVS::computeEpsToStateJacobianAna()
{
  ...
  { 
    ...
    const CFreal relaxDeriv = m_cellEpsRelaxDeriv[cellID];
    ...
    for (m_pertSol = 0; m_pertSol < m_nbrSolPnts; ++m_pertSol)
    {
      ...
      for (m_pertVar = 0; m_pertVar < m_nbrEqs; ++m_pertVar)
      {
        ...
        m_epsJacobian[m_pertSide][m_pertSol][m_pertVar] *= relaxDeriv;
      }
    }
  }
}
...
void ConvDiffLLAVJacobFluxReconstructionNSBaseVS::setup()
{
  CFAUTOTRACE;
  
  // setup parent class
  ConvDiffLLAVJacobFluxReconstructionVS::setup();
