...
// NS-specific diffusive Jacobian + blending: inherits from
// DiffRHSJacobFluxReconstructionBlendingVS (SFV-6: no alpha scaling) and adds NS-specific BR2 damping,
// viscosity-based wave speed, gradient variable transformation, and
// composition setting.  Same relationship as DiffRHSFluxReconstructionNSBlendingVS
// has to DiffRHSFluxReconstructionBlendingVS.
...
#include "FluxReconstructionNavierStokes/DiffRHSJacobFluxReconstructionNSBlendingVS.hh"
...
MethodCommandProvider< DiffRHSJacobFluxReconstructionNSBlendingVS,
		       FluxReconstructionSolverData,
		       FluxReconstructionNavierStokesModule >
diffRHSJacobNSBlendingVSFluxReconstructionProvider("DiffRHSJacobNSBlendingVS");

//////////////////////////////////////////////////////////////////////////////

DiffRHSJacobFluxReconstructionNSBlendingVS::DiffRHSJacobFluxReconstructionNSBlendingVS(const std::string& name) :
  DiffRHSJacobFluxReconstructionBlendingVS(name),
  m_tempGradTermL(),
  m_tempGradTermR(),
...
void DiffRHSJacobFluxReconstructionNSBlendingVS::computeWaveSpeedUpdates(vector< CFreal >& waveSpeedUpd)
{
...
void DiffRHSJacobFluxReconstructionNSBlendingVS::computeInterfaceFlxCorrection()
{
  ...
  for (CFuint iFlxPnt = 0; iFlxPnt < m_nbrFaceFlxPnts; ++iFlxPnt)
  {
    ...
    if (m_useBR2VS)
    {
      ...
      // LEFT flux: F_v(u_L, avgGrad_BR2)
      for (CFuint iVar = 0; iVar < m_nbrEqs; ++iVar)
      {
        m_avgSol[iVar] = (*(m_cellStatesFlxPnt[LEFT][iFlxPnt]))[iVar];
      }
      prepareFluxComputation();
      computeFlux(m_avgSol,m_avgGrad,m_unitNormalFlxPnts[iFlxPnt],0,m_tempFlux);

      // RIGHT flux: F_v(u_R, avgGrad_BR2)
      for (CFuint iVar = 0; iVar < m_nbrEqs; ++iVar)
      {
        m_avgSol[iVar] = (*(m_cellStatesFlxPnt[RIGHT][iFlxPnt]))[iVar];
      }
      prepareFluxComputation();
...
void DiffRHSJacobFluxReconstructionNSBlendingVS::computeBndGradTerms(RealMatrix& gradTerm, RealMatrix& ghostGradTerm)
{
  SafePtr< NavierStokesVarSet > navierStokesVarSet = m_diffusiveVarSet.d_castTo< NavierStokesVarSet >();
...
void DiffRHSJacobFluxReconstructionNSBlendingVS::computeCellGradTerm(RealMatrix& gradTerm)
{
  SafePtr< NavierStokesVarSet > navierStokesVarSet = m_diffusiveVarSet.d_castTo< NavierStokesVarSet >();
...
void DiffRHSJacobFluxReconstructionNSBlendingVS::computeFaceGradTerms(RealMatrix& gradTermL, RealMatrix& gradTermR)
{
  SafePtr< NavierStokesVarSet > navierStokesVarSet = m_diffusiveVarSet.d_castTo< NavierStokesVarSet >();
...
void DiffRHSJacobFluxReconstructionNSBlendingVS::prepareFluxComputation()
{
...
void DiffRHSJacobFluxReconstructionNSBlendingVS::setup()
{
  CFAUTOTRACE;

  // setup parent class
  DiffRHSJacobFluxReconstructionBlendingVS::setup();
