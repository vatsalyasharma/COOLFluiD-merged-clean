...
#include "FluxReconstructionNavierStokes/NSJacobGradientComputerBlendingVS.hh"
#include "FluxReconstructionNavierStokes/FluxReconstructionNavierStokes.hh"
#include "FluxReconstructionMethod/FluxReconstructionElementData.hh"
#include "NavierStokes/NavierStokesVarSet.hh"

//Vatsalya: new class; blending convective classes computed grad(rho_i) instead of grad(y_i) for NEQ; core FR plugin cannot include NS headers so setGradientVars() was missing from blending path; created NS-specific daughter class
...
MethodCommandProvider< NSJacobGradientComputerBlendingVS,
		       FluxReconstructionSolverData,
		       FluxReconstructionNavierStokesModule >
nsJacobGradientComputerBlendingVSProvider("ConvRHSJacobNSBlendingVS");

//////////////////////////////////////////////////////////////////////////////

NSJacobGradientComputerBlendingVS::NSJacobGradientComputerBlendingVS(const std::string& name) :
...
void NSJacobGradientComputerBlendingVS::computeGradients()
{
  // Gather state data pointers and reset grad updates
  ...
  for (CFuint iSolPnt = 0; iSolPnt < m_nbrSolPnts; ++iSolPnt)
  {
    ...
    for (CFuint iEq = 0; iEq < m_nbrEqs; ++iEq)
    {
      ...
      {
	// Use transformed gradient variable instead of raw state
...
void NSJacobGradientComputerBlendingVS::computeGradientFaceCorrections()
{
  // Gather flux point state data
  ...
  for (CFuint iFlx = 0; iFlx < m_nbrFaceFlxPnts; ++iFlx)
  {
    ...
    for (CFuint iEq = 0; iEq < m_nbrEqs; ++iEq)
    {
      // Use transformed gradient variables instead of raw state
...
void NSJacobGradientComputerBlendingVS::setup()
{
  CFAUTOTRACE;

  // setup parent class
  ConvRHSJacobFluxReconstructionBlending::setup();
