...
//Vatsalya: new class (DISABLED); providers commented out because enabling full boundary Jacobian destabilizes Newton for TCNEQ; solver falls back to non-Jacobian boundary class which accidentally stabilizes Newton
...
#include "FluxReconstructionNavierStokes/NSJacobBndGradientComputerBlendingVS.hh"
...
// DISABLED: These providers cause the solver to use full boundary Jacobian assembly
// instead of the RHS-only fallback (ConvBndCorrectionsRHSBlending). The full boundary
// Jacobian destabilizes Newton for TCNEQ. Re-enable when Newton convergence is stable.
//
// MethodCommandProvider< NSJacobBndGradientComputerBlendingVS,
//                        FluxReconstructionSolverData,
//                        FluxReconstructionNavierStokesModule >
// NSJacobBndGradientComputerBlendingVSProvider("ConvBndCorrectionsRHSJacobNSBlendingVS");
//
// MethodCommandProvider< NSJacobBndGradientComputerBlendingVS,
//                        FluxReconstructionSolverData,
//                        FluxReconstructionNavierStokesModule >
// NSJacobBndGradientComputerTurbBlendingVSProvider("ConvBndCorrectionsRHSJacobTurbBlendingVS");

//////////////////////////////////////////////////////////////////////////////

NSJacobBndGradientComputerBlendingVS::NSJacobBndGradientComputerBlendingVS(const std::string& name) :
...
void NSJacobBndGradientComputerBlendingVS::computeGradientBndFaceCorrections()
{
  ...
  else if ((getMethodData().getUpdateVarStr() == "Puvt" || (getMethodData().getUpdateVarStr() == "RhoivtTv" || getMethodData().getUpdateVarStr() == "Rhoivt")) && getMethodData().hasArtificialViscosity()) //Vatsalya: original checked only "RhoivtTv" (TCNEQ 2T); CNEQ uses "Rhoivt" (1T) which bypassed all NEQ-specific gradient/composition/physicality paths
  {
    // Loop over solution pnts to calculate the grad updates
    for (CFuint iSolPnt = 0; iSolPnt < m_nbrSolPnts; ++iSolPnt)
    {
...
void NSJacobBndGradientComputerBlendingVS::setup()
{
  CFAUTOTRACE;
