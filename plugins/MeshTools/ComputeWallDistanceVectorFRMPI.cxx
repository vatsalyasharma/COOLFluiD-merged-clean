  ...
  for (CFuint root = 0; root < m_nbProc; ++root) {
	...
	if (m_myRank == root) {
	  ...
	  nodeSize = nbNodesInTrs*dim;
          //Vatsalya: OLD block kept because MPI also uses connSize to build the broadcast datatype; overestimating 2-D faces read past the packed vector.
          ...
          //Vatsalya: New size matches the actual face-node entries pushed into trsNodeConn before MPI_Bcast.
